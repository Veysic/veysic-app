...
function Card({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <div style={{ backgroundColor: LIGHT, border: "1px solid rgba(31,31,31,0.1)", borderRadius: "6px", padding: "1.5rem", ...style }}>{children}</div>;
}
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
        <input type={type} value={value} onChange={(e) => onChange(e.target.value)} min={min} max={max} step={step} placeholder={placeholder}
          style={{ flex: 1, background: BG, border: "1px solid rgba(31,31,31,0.2)", borderRadius: "4px", padding: "0.6rem 0.75rem", fontSize: "0.9rem", color: DARK, outline: "none", fontFamily: "var(--font-inter), system-ui, sans-serif" }}
          onFocus={(e) => (e.target.style.borderColor = DARK)} onBlur={(e) => (e.target.style.borderColor = "rgba(31,31,31,0.2)")} />
  ...
  return (
    <div>
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        style={{ width: "100%", background: BG, border: "1px solid rgba(31,31,31,0.2)", borderRadius: "4px", padding: "0.6rem 0.75rem", fontSize: "0.85rem", color: DARK, fontFamily: "var(--font-inter), system-ui, sans-serif", outline: "none" }}>
...
function CTAReserva() {
  return (
    <div style={{ marginTop: "1.5rem", padding: "1rem 1.25rem", backgroundColor: "rgba(31,31,31,0.04)", borderRadius: "6px", border: "1px solid rgba(31,31,31,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "0.75rem" }}>
      <p style={{ fontSize: "0.78rem", color: MID, flex: 1 }}>¿Quieres un plan personalizado? Nuestros profesionales pueden ayudarte.</p>
      <Link href="/reservar" style={{ padding: "0.55rem 1.1rem", fontSize: "0.62rem", letterSpacing: "0.12em", textTransform: "uppercase", backgroundColor: DARK, color: BG, textDecoration: "none", borderRadius: "4px", whiteSpace: "nowrap" }}>
        Reservar sesión
      </Link>
    </div>
  );
...
function CalcIMC() {
  ...
  const categoria = imc === null ? null
    : imc < 18.5 ? { label: "Bajo peso", color: "#3498db", pct: 15 }
    : imc < 25 ? { label: "Peso normal", color: "#27ae60", pct: 38 }
    : imc < 30 ? { label: "Sobrepeso", color: "#e67e22", pct: 63 }
    : { label: "Obesidad", color: "#c0392b", pct: 88 };
    ...
    <Card>
      ...
      {imc !== null && categoria && (
        <div>
          ...
          <div style={{ position: "relative", height: 10, backgroundColor: "rgba(31,31,31,0.08)", borderRadius: 5, marginBottom: "0.5rem", overflow: "hidden" }}>
            <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, #3498db 0%, #27ae60 25%, #e67e22 60%, #c0392b 100%)", borderRadius: 5 }} />
            <div style={{ position: "absolute", top: -2, width: 14, height: 14, borderRadius: "50%", backgroundColor: "#fff", border: `3px solid ${categoria.color}`, transform: "translateX(-50%)", left: `${categoria.pct}%`, boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }} />
          </div>
          ...
          <p style={{ fontSize: "0.78rem", color: MID, lineHeight: 1.6, padding: "0.75rem", backgroundColor: `${categoria.color}10`, borderRadius: "4px", borderLeft: `3px solid ${categoria.color}` }}>
            {recomendacion}
          </p>
        </div>
      )}
  ...
  return (
    <Card>
      ...
      {tdee !== null && (
        <div>
          {[
            { label: "Mantenimiento", value: `${tdee} kcal`, sub: "Para mantener tu peso actual", color: "#27ae60" },
            { label: "Perder peso (déficit -500)", value: `${tdee - 500} kcal`, sub: "Para perder ~0,5 kg por semana", color: "#3498db" },
            { label: "Ganar músculo (superávit +300)", value: `${tdee + 300} kcal`, sub: "Para ganar masa muscular", color: "#8e44ad" },
          ].map(({ label, value, sub, color }) => (
            <div key={label} style={{ padding: "0.85rem 1rem", borderRadius: "6px", backgroundColor: `${color}10`, borderLeft: `3px solid ${color}`, marginBottom: "0.6rem" }}>
...
function Calc1RM() {
  ...
  return (
    <Card>
      ...
      {rm1 !== null && (
        <div>
          <div style={{ textAlign: "center", marginBottom: "1.25rem", padding: "1rem", backgroundColor: DARK, borderRadius: "6px" }}>
            <p style={{ fontSize: "0.62rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(237,230,217,0.6)", marginBottom: "0.25rem" }}>Tu 1RM estimado</p>
            <p style={{ fontSize: "2.5rem", fontWeight: 800, color: BG, lineHeight: 1 }}>{rm1} kg</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.5rem" }}>
            {porcentajes.map((pct) => (
              <div key={pct} style={{ textAlign: "center", padding: "0.6rem 0.25rem", backgroundColor: `rgba(31,31,31,${0.04 + (pct - 50) / 300})`, borderRadius: "4px", border: "1px solid rgba(31,31,31,0.08)" }}>
...
function CalcFC() {
  ...
  const zonas = fcMax === null ? [] : [
    { nombre: "Recuperación activa", rango: [Math.round(fcMax * 0.5), Math.round(fcMax * 0.6)], color: "#3498db", desc: "Warm-up, recuperación" },
    { nombre: "Aeróbico base", rango: [Math.round(fcMax * 0.6), Math.round(fcMax * 0.7)], color: "#27ae60", desc: "Quema grasa, resistencia" },
    { nombre: "Aeróbico intenso", rango: [Math.round(fcMax * 0.7), Math.round(fcMax * 0.8)], color: "#f39c12", desc: "Cardio, resistencia cardiovascular" },
    { nombre: "Anaeróbico", rango: [Math.round(fcMax * 0.8), Math.round(fcMax * 0.9)], color: "#e67e22", desc: "Potencia, velocidad" },
    { nombre: "Máximo", rango: [Math.round(fcMax * 0.9), fcMax], color: "#c0392b", desc: "Intervalos breves de alta intensidad" },
  ];
  ...
  return (
    <Card>
      ...
      {fcMax !== null && (
        <div>
          <div style={{ textAlign: "center", marginBottom: "1.25rem", padding: "0.75rem", backgroundColor: "#c0392b15", borderRadius: "6px", border: "1px solid #c0392b30" }}>
            <p style={{ fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "#c0392b", marginBottom: "0.2rem" }}>FC Máxima estimada</p>
            <p style={{ fontSize: "2rem", fontWeight: 800, color: "#c0392b" }}>{fcMax} ppm</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {zonas.map((z) => (
              <div key={z.nombre} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.7rem 0.85rem", borderRadius: "6px", backgroundColor: `${z.color}12`, borderLeft: `3px solid ${z.color}` }}>
                <div style={{ flex: 1 }}>
  ...
  return (
    <Card>
      ...
      {litros !== null && vasos !== null && (
        <div>
          <div style={{ textAlign: "center", padding: "1.5rem 1rem", backgroundColor: "rgba(52,152,219,0.08)", borderRadius: "8px", border: "1px solid rgba(52,152,219,0.2)", marginBottom: "1rem" }}>
            <p style={{ fontSize: "3.5rem", fontWeight: 800, color: "#3498db", lineHeight: 1 }}>{litros}L</p>
...
export default function HerramientasPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: BG }}>
      {/* Header */}
      <div style={{ borderBottom: "1px solid rgba(31,31,31,0.1)", backgroundColor: BG, position: "sticky", top: 0, zIndex: 10 }}>
        ...
        {/* Tab bar */}
        <div style={{ display: "flex", overflowX: "auto", borderTop: "1px solid rgba(31,31,31,0.06)" }} className="max-w-3xl mx-auto px-4">
          {CALCS.map((c) => (
            <button key={c.id} onClick={() => setActive(c.id)} style={{
              padding: "0.6rem 1rem", background: "none", border: "none", borderBottom: active === c.id ? `2px solid ${DARK}` : "2px solid transparent",
